...
EOF
        ...
        if [[ "$(uname -m)" == "x86_64" ]]; then
            sudo wget -4 -O /root/slipstream/slipstream-server 'https://raw.githubusercontent.com/ndevxlk/slipstream/main/slipstream-server-x86_64'
        else
            sudo wget -4 -O /root/slipstream/slipstream-server 'https://raw.githubusercontent.com/ndevxlk/slipstream/main/slipstream-server-aarch64'
        fi
...
ExecStart = /root/slipstream/slipstream-server --dns-listen-host 0.0.0.0 --dns-listen-port 53 --target-address 127.0.0.1:22 --domain a.team-whatsapp.com --cert cert.pem --key key.pem
Restart = always
[Install]
